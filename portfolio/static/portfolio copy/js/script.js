...
function renderCategoryFilters() {
  ...
  const uniqueCats = ['all', ...new Set(galleryItems.filter(i => i.visible).map(i => i.category))];
  ...
  filterContainer.querySelectorAll('.filter-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (swiperContainer) swiperContainer.classList.add('filtering');
      ...
      setTimeout(() => {
        ...
        if (swiperContainer) swiperContainer.classList.remove('filtering');
      }, 300);
    });
...
function renderGallery(filter) {
  if (typeof filter !== 'undefined') currentFilter = filter;
...
function initGallerySwiper() {
  ...
  gallerySwiper.on('click', function (swiper, event) {
    ...
    if (indexAttr !== null) {
      openLightbox(parseInt(indexAttr, 10));
    }
  });
...
if (uploadZone && fileInput && previewGrid && uploadForm) {
...
document.getElementById('uploadBtn')?.addEventListener('click', async () => {
  ...
  btn.disabled = false;
  btn.innerHTML = '<i class="fas fa-upload"></i> ADD TO GALLERY';
...
function updateLightbox() {
  ...
  setTimeout(() => {
    ...
    if (img.complete) img.classList.remove('changing');
...
async function tryLogin() {
  ...
  try {
    ...
    if (res.ok && data.status === 'success') {
      ...
      const fab = document.getElementById('adminFabBtn');
      if (fab) fab.innerHTML = '<span class="admin-fab-label">GALLERY ADMIN</span><i class="fas fa-images"></i>';
